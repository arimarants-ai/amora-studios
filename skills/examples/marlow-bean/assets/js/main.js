...
(function () {
  ...
  // The address enquiries go to. It is assembled here rather than written
  // into the markup so ordinary address scrapers, which read the HTML for
  // mailto: links, do not find it. A determined person still can — that is
  // unavoidable without a backend, and is why the database path exists.
  function inbox() {
  ...
  for (var m = 0; m < mailLinks.length; m++) {
    var subj = mailLinks[m].getAttribute("data-mailto") || "Hello the business";
